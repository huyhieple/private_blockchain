...
chaincodeQuery() {
    ...
    # Query Car by Id
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function": "queryCar","Args":["CAR34"]}'
...
createCar() {
    ...
    --peerAddresses localhost:7051 --tlsRootCertFiles $PEER0_ORG1_CA \
    --peerAddresses localhost:9051 --tlsRootCertFiles $PEER0_ORG2_CA \
    -c '{"function": "createCar","Args":["CAR34", "Mazda", "CX-5", "Red", "LHH"]}'
...
# chaincodeQueryAllCar() {
#     setGlobalsForPeer0Org2
#     result=$(peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function": "queryAllCars","Args":[]}')

#     # Kiem tra ket qua tra ve
#     if [ "$result" != "null" ]; then
#         echo "$result" | jq -r '.[] | [.Key, .Record.colour, .Record.make, .Record.model, .Record.owner] | @tsv' | column -t
#     else
